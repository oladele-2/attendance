...
export default async function OnDutyPage({
  ...
  const minForgotten = hours * 60;
  const forgotten = open.filter((row) => Number(row.minutes_open) >= minForgotten);
    ...
    <main className="mx-auto max-w-4xl px-4 py-8">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            {session.company} · {formatLongDate(today)} · Africa/Lagos
          </p>
        </div>
      </div>
      ...
      <section className="mb-8 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        <h2 className="border-b border-slate-100 px-4 py-3 text-sm font-semibold text-slate-700">
          Checked in today ({open.length})
        </h2>
        {open.length === 0 ? (
          <p className="p-6 text-sm text-slate-500">Nobody is on duty for today.</p>
        ) : (
              ...
              <li key={row.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500">
                    In {mysqlLagosStamp(row.check_in_time)} · {minutesToHm(Number(row.minutes_open))}
                  </p>
                </div>
              </li>
            ))}
      ...
      <section className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        ...
        {forgotten.length === 0 ? (
          <p className="p-6 text-sm text-slate-500">No open shifts today older than {hours} hours.</p>
        ) : (
          <ul className="divide-y divide-slate-100">
            {forgotten.map((row) => (
